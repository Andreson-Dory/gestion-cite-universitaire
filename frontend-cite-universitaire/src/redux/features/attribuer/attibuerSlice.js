  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAttribuer.fulfilled, (state, action) => {
        ...
        state.attribuers = action.payload.attribuers;
      ...
      .addCase(editAttribuer.fulfilled, (state, action) => {
        const index = state.attribuers.findIndex(
          (r) => r.IdAtt === action.payload.Attribuer.IdAtt,
        );
        if (index !== -1) {
          state.attribuers[index] = {
            ...state.attribuers[index],
            ...action.payload.Attribuer,
          };
        }
      })
      ...
      .addCase(removeAttribuer.fulfilled, (state, action) => {
        state.attribuers = state.attribuers.filter(
          (r) => r.IdAtt !== Number(action.payload.IdAtt),
        );
      });
  },
});
