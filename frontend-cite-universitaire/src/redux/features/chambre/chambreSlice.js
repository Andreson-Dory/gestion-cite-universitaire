  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchChambre.fulfilled, (state, action) => {
        ...
        state.chambres = action.payload.chambres;
      ...
      .addCase(editChambre.fulfilled, (state, action) => {
        const index = state.chambres.findIndex(
          (r) => r.IdCha === action.payload.Chambre.IdCha,
        );
        if (index !== -1) {
          state.chambres[index] = {
            ...state.chambres[index],
            ...action.payload.Chambre,
          };
        }
      })
      //delete a Chambre
      .addCase(removeChambre.fulfilled, (state, action) => {
        state.chambres = state.chambres.filter(
          (r) => r.IdCha !== Number(action.payload.IdCha),
        );
      });
  },
});
