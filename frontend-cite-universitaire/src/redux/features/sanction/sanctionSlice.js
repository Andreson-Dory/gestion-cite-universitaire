  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchSanction.fulfilled, (state, action) => {
        ...
        state.sanctions = action.payload.sanctions;
      ...
      .addCase(editSanction.fulfilled, (state, action) => {
        const index = state.sanctions.findIndex(
          (r) => r.IdSac === action.payload.Sanction.IdSac,
        );
        if (index !== -1) {
          state.sanctions[index] = {
            ...state.sanctions[index],
            ...action.payload.Sanction,
          };
        }
      })
      //delete a Sanction
      .addCase(removeSanction.fulfilled, (state, action) => {
        state.sanctions = state.sanctions.filter(
          (r) => r.IdSac !== Number(action.payload.IdSac),
        );
      });
  },
});
