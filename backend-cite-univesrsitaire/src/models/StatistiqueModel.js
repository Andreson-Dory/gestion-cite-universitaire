...
const getStatistique = async () => {
  ...
  const [BuildingCount] = await db.query(
    "SELECT COUNT(*) as count FROM Batiment;",
  );
  const [RoomCount] = await db.query("SELECT COUNT(*) as count FROM Chambre;");
  const [OccupedRooms] = await db.query(
    "SELECT COUNT(*) as count FROM Chambre WHERE StatutCha='Occupée'",
  );
  const [ReclamationCount] = await db.query(
    "SELECT COUNT(*) as count FROM Reclamation;",
  );
  const [PendingReclamationCount] = await db.query(
    "SELECT COUNT(*) as count FROM Reclamation WHERE StatusRec='En attente';",
  );
  const [SanctionCount] = await db.query(
    "SELECT COUNT(*) as count FROM Sanction",
  );
